...
**Setup**.  There are several packages we must load in order to conduct this analysis.  The `caret` package utilizes the createDataPartition function to create a series of test/training partitions.  The `randomForest` package allows us to use the randomForest function to build a classification model.  
